...
basedir = '/projects/b1108/studies/rise/data/raw/neuroimaging'; %where is your raw beh data
...
%% CHATROOM TASK
if chatroom == 1
    CRfnames_txt = filenames(fullfile(basedir,'/sub-*/ses-',num2str(ses),'/beh/chzc*txt'));
    CRfnames_csv = filenames(fullfile(basedir,'/sub-*/ses-',num2str(ses),'/beh/chzc*csv')); 
...
%%MID files
if mid == 1
    MIDfnames = filenames(fullfile(basedir,'sub-*/ses-',num2str(ses),'/beh/3_MID*txt'));
    ...
    tgt_onset1 = txt.Var2(matches(txt.Var1, 'Run1Tgt.OnsetTime')) ./1000; %motor period
    ...
    if length(cue_onset1) == length(cue_onset2)
        ...
        motor = ('motor_period',48,1)
    
        %compile variables into events.tsv file
        cue1 = [cue_onset1;cue_dur1;trial_type1] %cue onset timing, cue duration, anticipation of what trial type
        tgt1 = [tgt_onset1; tgt_dur1;motor] %target onset, target dur, motor
        fbk1 = [fbk_on1, fbk_dur1,fbk_type1] %feedback onset, feedback duration, trial type and hit/miss

        cue2 = [cue_onset2;cue_dur2;trial_type2] %cue onset timing, cue duration, anticipation of what trial type
        tgt2 = [tgt_onset2; tgt_dur2;motor] %target onset, target dur, motor
        fbk2 = [fbk_on2, fbk_dur2,fbk_type2] %feedback onset, feedback duration, trial type and hit/miss
        
        mid_events1 = array2table([cue1,motor1,fbk1]);
        mid_events1.Properties.VariableNames = {'onset', 'duration', 'trial_type'};
        mid_txt1 = fullfile(basedir, "/bids", strcat('sub-',pid,'/ses-',ses,'/func/sub-',pid,'_ses-',ses,'_task-mid_run-01_events.txt'));
            writetable(mid_events1,mid_txt1,'delimiter','tab')

        mid_events2 = array2table([cue2,motor2,fbk2]);
        mid_events2.Properties.VariableNames = {'onset', 'duration', 'trial_type'};
        mid_txt2 = fullfile(basedir, "/bids", strcat('sub-',pid,'/ses-',ses,'/func/sub-',pid,'_ses-',ses,'_task-mid_run-02_events.txt'));
